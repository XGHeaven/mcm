...
function buildCanonicalizedResource(resourcePath: string, parameters: string | string[] | Record<string, string>) {
...
function canonicalString(method: string, resourcePath: string, headers: Record<string, string>) {
...
function computeSignature(accessKeySecret: string, canonicalString: string) {
  return btoa(String.fromCharCode(...(hmac('sha1', accessKeySecret, canonicalString, 'utf8') as Uint8Array)))
};

export class AliOSSLayer extends StorageLayer {
  constructor(private key: string, private secret: string, private bucket: string, private region: string) {
    super()
  }

  private request(method: string, objectKey: string, data?: ArrayBuffer, type?: string) {
    ...
    if (data && method === 'PUT') {
      signHeader['content-md5'] = hash.createHash('md5').update(data).toString('base64')
    }

    const signResult = computeSignature(this.secret, canonicalString(
      method,
  ...
  async write(filepath: string, data: string | ArrayBuffer, options: LayerWriteOptions = {}): Promise<void> {
