...
export type TaskExecutor = (context: TaskContext) => Promise<void | ((children: ChildTasksStatus) => void)>
...
export function createTask(name: string, exe: TaskExecutor, parent: Task | null = null): Task {
  return {
    name,
    executor: exe,
...
export class SyncQueue {
  ...
  private run(task: Task) {
    ...
    result.then((finisher) => {
      ...
      finish()
      console.log(`${this.formatChildPercent(task.parent)}Task finished: ${task.name}`)
    }, e => {
      ...
      finish()
      console.error(`${this.formatChildPercent(task.parent)}Task error: ${task.name}`)
  ...
  private queueChild(parentTask: Task, name: string, executor: TaskExecutor) {
    ...
    this.tasks.splice(parentIndex === -1 ? this.tasks.length : parentIndex, 0, createTask(name, executor, parentTask))
  ...
  private finishChildTask(task: Task) {
    ...
    if (task.childCount <= task.childFinished && task.parent) {
      if (task.finisher) {
        this.tasks.unshift(createTask(`${task.name}:finisher`, async () => await task.finisher!({
