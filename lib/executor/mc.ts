...
const getAssetRemoteEndpoint = (hash: string) => `http://resources.download.minecraft.net/${hash.substring(0, 2)}/${hash}`
const getAssetStorageEndpoint = (hash: string) => `/minecraft/assets/${hash.substring(0, 2)}/${hash}`
...
export class MinecraftExecutor {
  ...
  createCacheAssetIndex(assetIndex: MinecraftPackageAssetIndex): TaskExecutor {
    return async ({queueChild, task}) => {
      ...
      if (assetIndex.sha1 !== assetIndexJsonHash) {
        console.error(`(${assetIndex.id}) Fetch asset index maybe not correct, hash is not matched.`)
        console.error(`  ${assetIndex.url}(expected) !== ${assetIndexJsonHash}(actual)`)
        console.error(`  Please restart sync again`)
        return;
      }

      // TODO: debug
      for (const {hash} of Object.values(json.objects).slice(0, 10) as any[]) {
        queueChild(`${task.name}:${hash}`, this.createCacheResource(getAssetRemoteEndpoint(hash), getAssetStorageEndpoint(hash)))
      }

      await this.storage.cacheFile(getAssetIndexStorageEndpoint(assetIndexUrl), data)
    }
  }
  ...
  createVersion(version: MinecraftVersion): TaskExecutor {
    ...
    return async ({queueChild, task}) => {
      ...
      queueChild(`${task.name}:assetIndex`, this.createCacheAssetIndex(mcPackage.assetIndex))

      // TODO: debug
      for (const [type, download] of Object.entries(mcPackage.downloads)) {
        queueChild(`${task.name}:downloads:${type}`, this.createCacheResource(download.url, getLauncherEndpoint(download.url)))
      }

      // TODO: debug
      for (const {downloads: libInfo, name: libName} of mcPackage.libraries) {
        queueChild(`${task.name}:library:${libName}`, this.library(libInfo.artifact))
        if (libInfo.classifiers) {
          for (const [type, lib] of Object.entries(libInfo.classifiers)) {
            queueChild(`${task.name}:library:${libName}:classifiers:${type}`, this.library(lib))
          }
        }
      }
      ...
      return async ({error}) => {
        if (error) {
          console.error(`Version ${version.id} has verified error, please try start again`)
          return;
        } else {
          await this.storage.cacheFile(getVersionLockEndpoint(version.id), new Uint8Array(0))
        }
      }
    }
  }
  ...
  createMinecraftVersion(version?: string | RegExp): TaskExecutor {
    return async ({queueChild, task}) => {
      ...
      if (typeof version === 'string') {
        const selectedVersion = manifest.versions.find(ver => ver.id === version)
        if (!selectedVersion) {
          console.error('Cannot found version of ' + version)
          return
        }
        queueChild(`${task.name}:${version}`, this.createVersion(selectedVersion))
      } else if (version) {
        // regexp
