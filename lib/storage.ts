...
export class StorageManager {
  static createLayer(): StorageLayer {
    ...
    switch (type) {
      case 'fs':
        return new FsLayer(path.resolve(Deno.cwd(), Deno.env.get('FS_LAYER_STORAGE') || '.storage'))
      case 'alioss':
  ...
  async cacheRemoteFile(source: string, target: string, force: boolean = false): Promise<void> {
  ...
  async cacheFile(target: string, data: Uint8Array, force = false) {
    ...
    await this.layer.write(target, data, {
      type: target.endsWith('.json') ? 'application/json' : 'application/octet-stream'
    })
  }
